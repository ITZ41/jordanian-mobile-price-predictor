...
from src.paths import CLEANED_DATA_PATH, MODEL_PATH, METRICS_PATH
...
feature_cols = [c for c in ['title', 'brand', 'series', 'storage_gb', 'phone_age_months', 'condition', 'model_name', 'color_actual'] if c in df.columns]
X = df[feature_cols]
y = df['price_jd']
y_log = np.log1p(y)
...
def r2_orig(estimator, X, y_log):
    y_true = np.expm1(y_log)
    y_pred = np.expm1(estimator.predict(X))
    return r2_score(y_true, y_pred)


def mae_orig(estimator, X, y_log):
    y_true = np.expm1(y_log)
    y_pred = np.expm1(estimator.predict(X))
    return mean_absolute_error(y_true, y_pred)


# --- Evaluate candidates with 5-fold CV ---
cv = KFold(n_splits=cfg('model.tuning.cv_folds', 5), shuffle=True, random_state=42)
best_name = None
best_cv_mae = float('inf')
best_pipeline = None

for name, pipe in candidates.items():
    log.info(f"Evaluating {name} with 5-fold CV...")
    scores = cross_validate(pipe, X, y_log, cv=cv, scoring={'r2': r2_orig, 'mae': mae_orig}, n_jobs=1)
    cv_mae = scores['test_mae'].mean()
    cv_r2 = scores['test_r2'].mean()
    log.info(f"  CV R2: {cv_r2:.3f} (+/- {scores['test_r2'].std():.3f})")
...
grid = GridSearchCV(best_pipeline, param_grid, cv=3, scoring='neg_mean_absolute_error', n_jobs=-1, verbose=1)
grid.fit(X, y_log)
...
log.info(f"Test Set Performance: MAE={mae:.2f} +/- {mae_std:.2f} JOD, R2={r2:.3f}")
